...
def CourseListView(request):
    ...
    VarDict = {'CourseObjAll': DisplayCourses, 'media': MEDIA_URL,
               'Tags': AllDiffTags, 'RecentCourses': RecentCourses, 'NotSearched': NotSearched, 'Filtered': False,
               'Language': 'null'}
    return render(request, 'Courses/coursesList.html', VarDict)
...
def HindiCourseListView(request):
    ...
    DisplayCourses = sorted(DisplayCourses, key=lambda Course: Course.Position)
    VarDict = {'CourseObjAll': DisplayCourses, 'media': MEDIA_URL,
               'Tags': AllDiffTags, 'RecentCourses': RecentCourses, 'NotSearched': NotSearched, 'Filtered': True,
               'Language': 'Hindi'}
    return render(request, 'Courses/coursesList.html', VarDict)
...
def SearchCourse(request):
    ...
    varDict = {'CourseObjAll': DisplayCourses, 'media': MEDIA_URL, 'NotSearched': NotSearched, 'Filtered': False,
               'Language': 'null'}
...
def SpecificCourseView(request, namo):
    ...
    """for i in OwnedCourses:
        videos = Course.objects.get(Name = namo)"""
    return render(request, 'Courses/single-blog.html',
                  {'namo': namo, 'videos': videos, 'media': MEDIA_URL, 'NotVerified': NotVerified})


def Pay1(request, CourseId):
    Prompt = "Please login to continue with your purchase"
    return render(request, 'Courses/login2.html', {'Prompt': Prompt, 'CourseId': CourseId, 'Media': MEDIA_URL})
        ...
        for i in Persons:
            if (i.Email == Email):
                if (i.Password == Password):
                    ...
                    return render(request, 'Courses/buy.html', {'Product': CourseObj, 'Media': MEDIA_URL, 'Person': i})
                else:
                    Prompt = "Wrong Password"
                    return render(request, 'Courses/login2.html', {'Prompt': Prompt})
            else:
                Prompt = "User Doesnt Exist, please register first!"
                continue
        return render(request, 'Courses/login2.html', {'Prompt': Prompt})
...
def Pay3PayU(request):
    ...
    hashSequence = "FEG7f40y|" + str(txnid) + "|" + str(price) + "|" + str(
        PId) + "|" + Namo + "|" + Person.Email + "|" + str(CId) + "|||||||||"
    ...
    return render(request, 'Courses/PayUForm.html', VarDict)
...
def PaymentSuccess(request):
    ...
    if (hashh != posted_hash):
        s = False
        vardict = {"txnid": txnid, "status": status, "amount": amount, "s": s, 'Product': Product, 'user': user,
                   'Media': MEDIA_URL}
        return render(request, 'Courses/SuccessPg.html', vardict)
